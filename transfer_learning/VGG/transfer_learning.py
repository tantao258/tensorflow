...
class Vgg16:
    ...
    def conv_layer(self, bottom, name):
        with tf.variable_scope(name):   # CNN's filter is constant, NOT Variable that can be trained
            conv = tf.nn.conv2d(input=bottom, filter=self.data_dict[name][0], strides=[1, 1, 1, 1], padding='SAME')
    ...
    def predict(self, paths):
        ...
        for i, path in enumerate(paths):
            x = load_img(path)
            length = self.sess.run(self.out, feed_dict={self.tfx: x})
            axs[i].imshow(x[0])
...
def train():
    ...
    for i in range(300):
        ...
        print(i, 'train loss: {}'.format(train_loss))

    vgg.save('./model/model.ckpt')      # save learned fc layers


def eval():
    vgg = Vgg16(vgg16_npy_path='./vgg16.npy',
                restore_from='./model/model.ckpt')
    vgg.predict(['./data/kittycat/000129037.jpg', './data/tiger/391412.jpg'])
